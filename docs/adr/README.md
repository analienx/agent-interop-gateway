...
The current architecture follows four foundational decisions:
...
4. **Read reasoning and machine authority are separate.** A first-class `constrained_agent` may reason over an explicit read tool plane, but mutation authority requires a separate executor/profile. See `0001-constrained-agent-authority-boundary.md`.
